...
namespace Timereg.Pages
{
    public partial class DashboardComponent : ComponentBase
    {
        ...
        public string yeartxt { get { return "Total timene for år " + YearId.ToString(); } }
        ...
        protected async System.Threading.Tasks.Task Load4()
        {
            ...
            timeregdataGetProjectsResult = timeregdataGetProjectsResult.Where(x => x.workyear == YearId);
            if (MonthId != 0)
            {
                GgetUserTimeV = timeregdataGetProjectsResult.Where(x => x.workmonth == MonthId);
                timeregdataGetTimeusedsResult = timeregdataGetTimeusedsResult.Where(x => x.workyear == YearId && x.workmonth == MonthId);
            ...
            getUserTimeV = null;
            getTimeusedsResult = timeregdataGetTimeusedsResult.GroupBy(x => new { x.userid, x.workmonth, x.workyear, x.username })
                            .Select(c => new VUserWithTimeVM
                            {
                                userid = c.Key.userid,
                                username = c.Key.username,
                                ...
                                UserTime = (Math.Floor(100 * c.Sum(r => r.usertime) / 60.00)) / 100,
