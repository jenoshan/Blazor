...
namespace Timereg.Controllers
{
    ...
    {
        private readonly TimeregdataService context;

        public DownloadController(TimeregdataService context)
        {
            this.context = context;
        }
        [HttpGet("/export/Timereg/TimeusedPeruser/csv")]
        public async Task<FileStreamResult> ExportTimeusedPeruserToCSV()
        {
            var lst = await context.GetVTimeusedPeruser();
            return ToCSV<VTimeusedPeruser>(lst.ToList());
        }

        [HttpGet("/export/Timereg/TimeusedPeruser/excel")]
        public async Task<FileStreamResult> ExportTimeusedPeruserToExcel()
        {
            var lst = await context.GetVTimeusedPeruser();
            return ToExcel<VTimeusedPeruser>(lst.ToList()); 
        }
        ...
        public FileStreamResult ToExcel<T>(IEnumerable<T> entities)
        {
            ...
            using (var document = SpreadsheetDocument.Create(stream, SpreadsheetDocumentType.Workbook))
            {
                ...
                foreach (var item in entities)
                {
                    ...
                    foreach (var column in columns)
                    {
                        ...
                        var underlyingType = column.PropertyType.IsGenericType &&
                            column.PropertyType.GetGenericTypeDefinition() == typeof(Nullable<>) ?
        ...
        public FileStreamResult ToCSV<T>(IEnumerable<T> entities)
        {
            var columns = typeof(T).GetProperties()
              .Where(x => x.CanWrite)
              .ToList();
            ...
            foreach (var item in entities)
            {
                ...
                foreach (var column in columns)
                {
                    row.Add($"{GetValue(item, column.Name)}".Trim());
                }
            ...
            var result = new FileStreamResult(new MemoryStream(UTF8Encoding.Default.GetBytes($"{string.Join(",", columns.Select(c => c.Name))}{System.Environment.NewLine}{sb.ToString()}")), "text/csv");
            result.FileDownloadName = "Export.csv";
